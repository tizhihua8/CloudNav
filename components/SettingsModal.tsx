...
import { X, Save, Bot, Key, Globe, Sparkles, PauseCircle, Wrench, Box, Copy, Check, LayoutTemplate, RefreshCw, Info, Download, Sidebar, Keyboard, MousePointerClick, AlertTriangle, Package, Zap, Menu, PieChart, Activity, ShieldCheck, Clock, Trash, Loader2 } from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({
    ...
    const [scanTimeout, setScanTimeout] = useState(5000); // 默认 5000ms
    const [scanResults, setScanResults] = useState<{ id: string, status: 'ok' | 'error' | 'timeout' }[]>([]);
    const [scanProgress, setScanProgress] = useState({ current: 0, total: 0 });
    const cancelScanRef = useRef(false);

    const [localSiteSettings, setLocalSiteSettings] = useState<SiteSettings>(() => ({
        title: siteSettings?.title || '十四的导航',
        navTitle: siteSettings?.navTitle || '十四 CloudNav',
        favicon: siteSettings?.favicon || '',
        cardStyle: siteSettings?.cardStyle || 'detailed',
        customPassword: siteSettings?.customPassword || ''
    }));
    ...
    // --- Cleanup & User Persona Functions ---
    const handleCheckLinks = async () => {
        if (isScanning) return;
        setIsScanning(true);
        cancelScanRef.current = false;
        setScanResults([]);
        setScanProgress({ current: 0, total: links.length });

        const results: { id: string, status: 'ok' | 'error' | 'timeout' }[] = [];
        ...
        for (let i = 0; i < links.length; i++) {
            ...
            try {
                ...
                // 使用 no-cors 模式尝试探测，虽然无法读内容，但能感知基础连通性
                // 某些网站禁用了 HEAD，改用 GET 但只取头部
                ...
                results.push({ id: link.id, status: 'ok' });
            } catch (err: any) {
                if (err.name === 'AbortError') {
                    results.push({ id: link.id, status: 'timeout' });
                } else {
                    results.push({ id: link.id, status: 'error' });
                }
            }

            setScanResults([...results]);
            setScanProgress({ current: i + 1, total: links.length });
        }

        setIsScanning(false);
    };

    const handleRemoveBrokenLinks = () => {
        const brokenIds = new Set(scanResults.filter(r => r.status !== 'ok').map(r => r.id));
        if (brokenIds.size === 0) {
            alert("未发现需要清理的失效链接");
            return;
        }

        if (confirm(`确定要删除这 ${brokenIds.size} 个失效链接吗？删除后点击“保存配置”将同步到云端及扩展插件。`)) {
            const newLinks = links.filter(l => !brokenIds.has(l.id));
            onUpdateLinks(newLinks);
            setScanResults([]);
            setScanProgress({ current: 0, total: 0 });
            alert("已在本地清理，请记得点击右下角“保存配置”同步到云端。");
        }
    };
    ...
    const userStats = useMemo(() => {
        return {
            ...
            avgPerCat: (links.length / (categories.length || 1)).toFixed(1),
            mostPopCat: [...categories].sort((a, b) =>
                links.filter(l => l.categoryId === b.id).length - links.filter(l => l.categoryId === a.id).length
            )[0]?.name || '无'
        };
    }, [links, categories]);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const tabs = [
        ...
        { id: 'data', label: '数据与清洗', icon: PieChart },
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden border border-slate-200 dark:border-slate-700 flex max-h-[90vh] flex-col md:flex-row">
                ...
                <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden bg-white dark:bg-slate-800">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 pb-12">
                        ...
                        {activeTab === 'site' && (
                            <div className="space-y-6 animate-in fade-in duration-300">
                                ...
                                <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                                    ...
                                    <input
                                        type="password"
                                        value={localSiteSettings.customPassword}
                                        onChange={(e) => handleSiteChange('customPassword', e.target.value)}
                                        placeholder="留空则使用部署时的 PASSWORD 环境变量"
                                    ...
                                    />
                                    <p className="text-[10px] text-slate-400 mt-1">设置后将覆盖环境变量密码。此密码仅用于 API 验证，不会在前端明文展示。</p>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="space-y-6 animate-in fade-in duration-300">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder={localConfig.provider === 'gemini' ? "gemini-2.0-flash" : "gpt-3.5-turbo"}
                                        className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                </div>
                        ...
                        {/* 2.5 Data Management & Cleanup */}
                        {activeTab === 'data' && (
                            <div className="space-y-6 animate-in fade-in duration-300">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
                                        <div className="text-xs text-blue-600 dark:text-blue-400 mb-1">书签总量</div>
                                        <div className="text-2xl font-bold text-blue-700 dark:text-blue-300">{userStats.totalLinks}</div>
                                    </div>
                                    <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl border border-purple-100 dark:border-purple-800">
                                        <div className="text-xs text-purple-600 dark:text-purple-400 mb-1">已分类</div>
                                        <div className="text-2xl font-bold text-purple-700 dark:text-purple-300">{userStats.totalCats} 个</div>
                                    </div>
                                    <div className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-100 dark:border-amber-800">
                                        <div className="text-xs text-amber-600 dark:text-amber-400 mb-1">常用分类</div>
                                        <div className="text-2xl font-bold text-amber-700 dark:text-amber-300 truncate">{userStats.mostPopCat}</div>
                                    </div>
                                </div>
                                ...
                                <div className="p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">自动探测失效、超时或无法访问的链接。</p>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <label className="text-xs text-slate-500">超时(ms):</label>
                                            <input
                                                type="number"
                                                value={scanTimeout}
                                                onChange={(e) => setScanTimeout(Number(e.target.value))}
                                                className="w-16 p-1 text-xs border border-slate-300 dark:border-slate-600 dark:bg-slate-700 rounded outline-none shadow-sm"
                                            />
                                        </div>
                                    </div>

                                    {isScanning ? (
                                        <div className="space-y-3 p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800">
                                            <div className="flex justify-between text-xs">
                                                <span className="flex items-center gap-2">
                                                    <Loader2 className="w-3 h-3 animate-spin text-blue-500" />
                                                    正在探测: {scanProgress.current}/{scanProgress.total}
                                                </span>
                                                <button onClick={() => cancelScanRef.current = true} className="text-red-500 hover:underline">取消</button>
                                            </div>
                                            <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                                <div className="h-full bg-blue-500" style={{ width: `${(scanProgress.current / scanProgress.total) * 100}%` }}></div>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="flex gap-3">
                                            <button
                                                ...
                                                className="flex-1 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 shadow-md hover:shadow-lg"
                                            >
                                                <Activity size={16} /> 开始全量扫描
                                            </button>

                                            {scanResults.length > 0 && (
                                                <button
                                                    onClick={handleRemoveBrokenLinks}
                                                    className="px-4 py-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-lg text-sm font-medium hover:bg-red-100 transition-colors flex items-center gap-2"
                                                >
                                                    <Trash size={16} /> 清除异常 ({scanResults.filter(r => r.status !== 'ok').length})
                                                </button>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    {scanResults.length > 0 && !isScanning && (
                                        <div className="max-h-40 overflow-y-auto border border-slate-200 dark:border-slate-700 rounded-lg divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-900">
                                            {links.filter(l => scanResults.find(r => r.id === l.id && r.status !== 'ok')).map(link => (
                                                <div key={link.id} className="p-2 flex items-center justify-between text-[11px]">
                                                    <span className="truncate text-slate-600 dark:text-slate-400 max-w-[70%]">{link.title}</span>
                                                    <span className="text-red-500 flex items-center gap-1 shrink-0">
                                                        <AlertTriangle size={10} />
                                                        {scanResults.find(r => r.id === link.id)?.status === 'timeout' ? '请求超时' : '接入异常'}
                                                    </span>
                                                </div>
                                            ))}
                                        </div>
